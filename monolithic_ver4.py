...
def setup_loggers():
    ...
    sim_formatter = KSTFormatter('%(asctime)s - %(message)s')
...
class GroundStation:
    ...
    async def try_aggregate_and_update(self):
        ...
        # <<< 수정된 부분: 객체 ID를 사용하여 처리된 모델을 안전하게 제거
...
async def main():
    ...
    if len(sat_ids) < num_masters * (sats_per_plane / num_masters): # TLE 파일의 plane 개수에 맞게 수정
...
if __name__ == "__main__":
    try:
        ...
        # __main__에서는 로거가 설정되지 않았을 수 있으므로, 다시 불러옵니다.
