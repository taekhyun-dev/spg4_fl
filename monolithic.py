...
def setup_loggers():
    ...
    # CSV 헤더 작성
    perf_handler.stream.write("timestamp,event_type,owner_id,model_version,accuracy,loss\n")
...
class SimulationClock:
    ...
    def __init__(self, start_dt: datetime, speed_multiplier: float = 1.0, sim_logger=None):
        self._current_dt = start_dt
        self.time_step = timedelta(seconds=1)
        self.speed_multiplier = speed_multiplier
        self.real_interval = 1.0 / speed_multiplier
        self.logger = sim_logger
        self.logger.info(f"시뮬레이션 시계 생성. 시작: {start_dt}, 속도: x{speed_multiplier}")
...
class GroundStation:
    ...
    async def run(self, clock: SimulationClock, satellites: Dict[int, 'Satellite']):
        ...
        while True:
            ...
            for sat_id, sat in satellites.items():
                ...
                if not prev_visible and visible_now: # AOS
                    self.logger.info(f"📡 [AOS] GS <-> MasterSAT {sat_id} 통신 시작 (고도각: {elevation:.2f}°)")
                    sat.state = 'COMMUNICATING_GS'
                    await self.send_model_to_satellite(sat)
                elif prev_visible and not visible_now: # LOS
                    self.logger.info(f"📡 [LOS] GS <-> MasterSAT {sat_id} 통신 종료 (고도각: {elevation:.2f}°)")
                    sat.state = 'IDLE'
                elif visible_now and sat.model_ready_to_upload:
    ...
    async def send_model_to_satellite(self, satellite: 'MasterSatellite'):
        self.logger.info(f"  📤 GS -> MasterSAT {satellite.sat_id}: 글로벌 모델 전송 (버전 {self.global_model.version})")
    ...
    async def receive_model_from_satellite(self, satellite: 'MasterSatellite'):
        ...
        if cluster_model:
            self.logger.info(f"  📥 GS <- MasterSAT {satellite.sat_id}: 클러스터 모델 수신 완료 (버전 {cluster_model.version}, 학습자: {cluster_model.trained_by})")
    ...
    async def aggregate_models_periodically(self):
        while True:
            ...
            self.logger.info(f"✨ [GS Aggregation] {len(models_to_aggregate)}개의 클러스터 모델(버전 >= {version_lower_bound})과 기존 글로벌 모델 Aggregation 시작...")
            ...
            self.logger.info(f"✨ [GS Aggregation] 새로운 글로벌 모델 생성 완료! (버전 {self.global_model.version})")
            ...
            self.perf_logger.info(f"{datetime.now(timezone.utc).isoformat()},GLOBAL_TEST,{self.name},{self.global_model.version},{accuracy:.4f},{loss:.6f}")
            ...
            if self.received_models_buffer:
                try:
                    ...
                    if models_to_discard:
                        ...
                        self.logger.info(f"  🗑️  [GS] {len(models_to_discard)}개의 오래된 모델 정리 (버전: {discard_versions})")
...
class Satellite:
    ...
    async def train_local_model(self):
        ...
        if self.state == 'TRAINING':
            self.logger.info(f"  🧠 SAT {self.sat_id}: 로컬 학습 완료.")
            ...
            self.perf_logger.info(f"{datetime.now(timezone.utc).isoformat()},LOCAL_VALIDATION,{self.sat_id},{self.local_model.version},{accuracy:.4f},{loss:.6f}")
...
class MasterSatellite(Satellite):
    ...
    async def receive_global_model(self, model: PyTorchModel):
        if model.version >= self.local_model.version:
            ...
            self.model_ready_to_upload = False
            self.logger.info(f"  🛰️  MasterSAT {self.sat_id}: 글로벌 모델 수신 (버전 {model.version}). 클러스터에 전파 시작.")
        else:
    ...
    async def _aggregate_and_evaluate_cluster_models(self):
        ...
        self.logger.info(f"  🧪 [Cluster Test] Owner: SAT_{self.sat_id}, Version: {self.local_model.version}, Accuracy: {accuracy:.2f}%, Loss: {loss:.4f}")
        self.perf_logger.info(f"{datetime.now(timezone.utc).isoformat()},CLUSTER_TEST,SAT_{self.sat_id},{self.local_model.version},{accuracy:.4f},{loss:.6f}")
...
async def main():
    ...
    simulation_clock = SimulationClock(start_time, speed_multiplier=600, sim_logger=sim_logger)
    ...
    ground_station = GroundStation("Seoul-GS", 37.5665, 126.9780, 34, initial_model=initial_global_model, eval_infra=eval_infra)
    ...
    sat_ids = list(all_sats_skyfield.keys())
    
    num_masters = 3
    if len(sat_ids) < num_masters + 1:
        raise ValueError("시뮬레이션을 위해 최소 4개의 위성 TLE가 필요합니다.")
        
    master_ids = sat_ids[:num_masters]
    worker_ids = sat_ids[num_masters:]
    ...
    tasks = [
        ...
        asyncio.create_task(ground_station.run(simulation_clock, satellites_in_sim)),
